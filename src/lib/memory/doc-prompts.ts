 ...
 * with two different audiences (Jason-the-SME vs. customer-flow),
 * and decoupling means we can iterate on prompts without touching
 * the schema work the agent reads from.
 */
...
export type DocPrompt = {
   ...
   *  Jason chat suggestion). Lowercase, no period. */
  shortLabel: string;
};
...
export const SECTION_DOC_PROMPTS: Partial<Record<MemoryFileSlug, DocPrompt>> = {
  ...
  brand_voice: {
    prompt:
      "Got brand guidelines or a style guide? Drop them here so Jason matches your colors, type, and tone exactly.",
    examples: ["brand guidelines", "style guide", "logo files", "tone-of-voice doc"],
    shortLabel: "brand guidelines or style guide",
  },
  competitor_landscape: {
    prompt:
      "Got notes on your competitors? Drop them here and Jason will turn them into a clean competitor brief.",
    examples: ["competitor analysis", "market research", "SWOT slides"],
    shortLabel: "competitor notes",
  },
  market_strategy: {
    prompt:
      "Got a marketing plan or go-to-market doc? Upload it and Jason will pull positioning + audience straight from it.",
    examples: ["marketing plan", "go-to-market doc", "marketing calendar"],
    shortLabel: "marketing plan",
  },
  unit_economics: {
    prompt:
      "Got financials? A P&L, a COGS breakdown, or an investment summary will let Jason fill in the unit economics without you re-typing the numbers.",
    examples: ["P&L", "COGS breakdown", "investment summary", "revenue model"],
    shortLabel: "P&L or COGS",
  },
  franchise_economics: {
    prompt:
      "Have you started an FDD, royalty schedule, or fee structure already? Drop it here and Jason can mirror it.",
    examples: ["existing FDD", "royalty schedule", "fee structure", "franchise sales deck"],
    shortLabel: "existing FDD or fee structure",
  },
  operating_model: {
    prompt:
      "Got an operations manual or opening checklist? Drop it here and Jason answers most of this section.",
    examples: [
      "operations manual",
      "opening checklist",
      "closing procedures",
  ...
  recipes_and_menu: {
    prompt:
      "Got your menu, product catalog, or recipe book? Upload it so Jason captures it as the official spec.",
    examples: ["menu PDF", "product catalog", "recipe book", "spec sheets"],
    shortLabel: "menu or product catalog",
  },
  vendor_supply_chain: {
    prompt:
      "Got an approved-supplier list or vendor roster? Drop it here and Jason will codify the supply chain.",
    examples: ["vendor list", "supplier agreements", "supply chain doc"],
    shortLabel: "vendor list",
  },
  training_program: {
    prompt:
      "Got a training manual, onboarding curriculum, or transcripts of your training videos? Drop them here and Jason builds a training program around them.",
    examples: ["training manual", "onboarding curriculum", "video transcripts", "LMS exports"],
    shortLabel: "training manual",
  },
  employee_handbook: {
    prompt:
      "Got an employee handbook or HR policies? Drop them here so Jason can codify the standards franchisees inherit.",
    examples: ["employee handbook", "HR policies", "code of conduct", "PTO policy"],
    shortLabel: "employee handbook",
  },
  franchisee_profile: {
    prompt:
      "Got an ideal-franchisee profile or applicant scorecard? Upload it and Jason will reflect your screening criteria exactly.",
  ...
  territory_real_estate: {
    prompt:
      "Got site selection criteria or a real-estate playbook? Drop it here and Jason will build territory + site rules around it.",
    examples: ["site selection criteria", "real estate playbook", "territory map"],
    shortLabel: "site selection criteria",
  },
  marketing_fund: {
    prompt:
      "Got a marketing-fund agreement or ad-fund spend report? Drop it here so Jason mirrors the governance you already use.",
    examples: ["marketing fund agreement", "ad fund spend report", "co-op rules"],
    shortLabel: "marketing fund agreement",
  },
  compliance_legal: {
    prompt:
      "Got an existing FDD, a franchise agreement template, or letters from your attorney? Drop them here and Jason will mirror the legal posture.",
    examples: ["existing FDD", "franchise agreement", "attorney letter", "state registration"],
    shortLabel: "FDD or attorney letter",
  },
  reimbursement_policy: {
    prompt:
      "Got an existing reimbursement policy or expense doc? Drop it here so Jason can codify what franchisees can claim.",
